...
auto_install_wizard() {
    ...
    if ! command -v sshpass &> /dev/null; then
        missing_tools+=("sshpass")
        echo "⚠️  sshpass가 설치되지 않았습니다. (선택사항)"
    else
    ...
    if [[ ${#missing_tools[@]} -gt 0 ]]; then
        ...
        for tool in "${missing_tools[@]}"; do
            case "$tool" in
                "gum")
                    echo "  - gum: 터미널 UI 라이브러리 (필수)"
                    ;;
                "sshpass")
                    echo "  - sshpass: SSH 패스워드 자동입력 (권장)"
                    ;;
            esac
        done
        echo ""
...
ensure_sshpass() {
    ...
    if command -v gum &> /dev/null; then
        ...
        if gum confirm "sshpass를 자동으로 설치하시겠습니까?"; then
            ...
            echo ""
            echo "⏭️  자동 설치를 건너뜁니다." | gum style --foreground 8 --align center
            echo "   패스워드를 수동으로 입력해야 합니다." | gum style --foreground 8 --align center
            echo ""
            return 1
        fi
